...
type TriggerModel struct {
	...
	AloneMetrics []string `json:"alone_metrics"`
}

// ToMoiraTrigger transforms TriggerModel to moira.Trigger
func (model *TriggerModel) ToMoiraTrigger() *moira.Trigger {
	aloneMetrics := make(map[string]bool, len(model.AloneMetrics))
	for _, targetName := range model.AloneMetrics{
		aloneMetrics[targetName] = true
	}
...
func (trigger *Trigger) Bind(request *http.Request) error {
	...
	for _, targetName := range trigger.AloneMetrics {
