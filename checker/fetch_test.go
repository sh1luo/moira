	...
	Convey("Test fetch trigger metrics", t, func() {
		...
		Convey("no metrics in last check", func() {
			Convey("fetch returns wildcard", func() {
				...
				database.EXPECT().RemoveMetricsValues([]string{pattern}, until-triggerChecker.config.MetricsTTLSeconds).Return(nil)

				actual, err := triggerChecker.fetchTriggerMetrics()
				So(err, ShouldResemble, ErrTriggerHasOnlyWildcards{})
				So(actual, ShouldResemble, metricSource.FetchedMetrics{"t1": metricSource.FetchedPatternMetrics{{Name: pattern, Wildcard: true}}})
			})
...
func TestFetch(t *testing.T) {
	...
	Convey("Test has metrics", t, func() {
		Convey("Only one target", func() {
			...
			expected := metricSource.FetchedMetrics{"t1": []metricSource.MetricData{metricData}}
			So(err, ShouldBeNil)
		...
		Convey("Two targets", func() {
			...
			metricData := metricSource.FetchedPatternMetrics{*metricSource.MakeMetricData(metric, []float64{0, 1, 2, 3, 4}, retention, from)}
			addMetricData := metricSource.FetchedPatternMetrics{*metricSource.MakeMetricData(addMetric, []float64{0, 1, 2, 3, 4}, retention, from)}
			...
			actual, metrics, err := triggerChecker.fetch()
			expected := metricSource.FetchedMetrics{"t1": metricData, "t2": addMetricData}
		...
		Convey("Two targets with many metrics in additional target", func() {
			...
			actual, metrics, err := triggerChecker.fetch()
			So(err, ShouldBeNil)
			So(actual, ShouldResemble, metricSource.FetchedMetrics{"t1": metricData, "t2": addMetricData})
