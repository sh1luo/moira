...
func TestGetMetricDataState(t *testing.T) {
	...
	metrics := metricSource.MetricsToCheck{
		"t1": metricT1,
		"t2": metricT2,
	}
	...
	Convey("Prepare metrics for check:", t, func() {
		...
		Convey("last check has no metrics", func() {
			Convey("fetched metrics is empty", func() {
				prepared, alone, err := triggerChecker.prepareMetrics(metricSource.FetchedMetrics{})
			...
			Convey("fetched metrics has metrics", func() {
				...
				fetched := metricSource.FetchedMetrics{
					"t1": metricSource.FetchedPatternMetrics{
			...
			Convey("fetched metrics has duplicate metrics", func() {
				...
				prepared, alone, err := triggerChecker.prepareMetrics(fetched)
				So(prepared, ShouldHaveLength, 0)
				So(alone, ShouldResemble, metricSource.MetricsToCheck{"t1": *metricSource.MakeMetricData("123", []float64{1, 2, 3}, 10, 0)})
			...
			Convey("Targets have different metrics", func() {
				fetched := metricSource.FetchedMetrics{
					...
					"t2": metricSource.FetchedPatternMetrics{
						*metricSource.MakeMetricData("second.metric", []float64{4, 5, 6}, 10, 0),
						*metricSource.MakeMetricData("third.metric", []float64{4, 5, 6}, 10, 0),
					},
				}
...
func TestGetMetricStepsStates(t *testing.T) {
	...
	Convey("ValueTimestamp covers all metric range", t, func() {
		...
		Convey("Metric has all valid values", func() {
			_, metricStates, err := triggerChecker.getMetricStepsStates("main.metric", metricSource.MetricsToCheck{"t1": metricData2, "t2": addMetricData})
			So(err, ShouldBeNil)
		...
		Convey("Metric has invalid values", func() {
			_, metricStates, err := triggerChecker.getMetricStepsStates("main.metric", metricSource.MetricsToCheck{"t1": metricData1, "t2": addMetricData})
			So(err, ShouldBeNil)
...
func TestIgnoreNodataToOk(t *testing.T) {
	...
	aloneMetrics := metricSource.MetricsToCheck{"t1": *metricSource.MakeMetricData(metric, []float64{0, 1, 2, 3, 4}, retention, triggerChecker.from)}
	triggerChecker.lastCheck.MetricsToTargetRelation = aloneMetrics.GetRelations()
	metricsToCheck := metricSource.TriggerMetricsToCheck{}
...
func TestHandleTrigger(t *testing.T) {
	...
	Convey("First Event", t, func() {
		aloneMetrics := metricSource.MetricsToCheck{"t1": *metricSource.MakeMetricData(metric, []float64{0, 1, 2, 3, 4}, retention, triggerChecker.from)}
		lastCheck.MetricsToTargetRelation = aloneMetrics.GetRelations()
		checkData := newCheckData(&lastCheck, triggerChecker.until)
		metricsToCheck := metricSource.TriggerMetricsToCheck{}
	...
	Convey("No data too long", t, func() {
		...
		dataBase.EXPECT().PushNotificationEvent(&moira.NotificationEvent{
			...
			Values:    map[string]float64{},
			Message:   nil}, true).Return(nil)
		aloneMetrics := metricSource.MetricsToCheck{"t1": *metricSource.MakeMetricData(metric, []float64{}, retention, triggerChecker.from)}
		lastCheck.MetricsToTargetRelation = aloneMetrics.GetRelations()
		checkData := newCheckData(&lastCheck, triggerChecker.until)
		metricsToCheck := metricSource.TriggerMetricsToCheck{}
